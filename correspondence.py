...
from utils import SparseMatrixCache, TriangleMarkersCache
...
def get_correspondence():
    ...
    def compute_adjacent_by_edges(mesh: meshlib.Mesh):
        ...
        faces_sorted = sorted([(f, [a for a in adj if a != f]) for f, adj in faces_adjacent.items()],
                              key=lambda e: e[0])
    ...
    def get_vertex_normals(verts: np.array, faces: np.array):
        ...
        vertex_normals = [np.mean(normals, 0) if normals else np.zeros(3, float) for normals in
                          triangle_normals_per_vertex]
    ...
    def construct_identity_cost(subject, transforms) -> Tuple[sparse.spmatrix, np.ndarray]:
        ...
        if AEi is None:
            ...
            for index, Ti in enumerate(
                    tqdm.tqdm(transforms, desc="Building Identity Cost")):  # type: int, TransformEntry
    ...
    matched_triangles = match_triangles(vertices, target_mesh.vertices, original_source.faces, target_mesh.faces,
                                        target_normals)
    cache = TriangleMarkersCache(suffix="_tri_markers").entry(hashid=hashid)
    cache.store(matched_triangles)
